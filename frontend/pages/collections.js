...
import { sampleCollections } from '@/utils/mockData';

export default function CollectionsPage() {
  const pendingCases = sampleCollections.filter((item) => item.status !== 'completed').length;
    ...
    <>
      ...
      <div className="space-y-6">
        <div className="grid gap-4 md:grid-cols-3">
          <StatCard label="Collection cases" value={sampleCollections.length} detail="Demo rows shown below" accent="sky" />
              ...
              <div key={item.id} className="rounded-3xl border border-slate-800 bg-white/5 p-5">
                <div className="flex items-start justify-between gap-3">
                  <div>
                    ...
                    <h3 className="mt-1 text-xl font-semibold text-white text-display">{item.loan}</h3>
                  </div>
