  ...
  return (
    <div className="rounded-3xl border border-slate-800/80 bg-slate-950/70 p-5 shadow-glow">
      <div className="flex items-start justify-between gap-4">
        <div>
          <p className="text-sm text-slate-400">{investment.loanName}</p>
          <h3 className="mt-1 text-lg font-semibold text-white text-display">{investment.seller}</h3>
        </div>
        <span className="rounded-full border border-emerald-400/20 bg-emerald-400/10 px-3 py-1 text-xs font-semibold text-emerald-200">
          {formatPercent(investment.ownership)} ownership
        </span>
      </div>
      ...
      <div className="mt-4 grid grid-cols-2 gap-3 text-sm text-slate-300">
        ...
        <div className="rounded-2xl bg-white/5 p-3">
          ...
          <p className="mt-1 font-semibold text-white">{investment.investmentId}</p>
        </div>
      </div>
